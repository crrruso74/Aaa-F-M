...
async def forcesub(c, m):
    owner = await c.get_users(int(5650200786))
    if iPopkarnBots:
        try:
            user = await c.get_chat_member(iPopkarnBots, m.from_user.id)
            if user.status == "kicked":
               await m.reply_text("**Hey you are banned 😜**", quote=True)
               return
        except UserNotParticipant:
            buttons = [[InlineKeyboardButton(text='Updates Channel 🍿', url=f"https://t.me/iPopkarnBots")]]
            if m.text:
